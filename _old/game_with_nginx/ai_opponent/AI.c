...
#define GAME_API_URL "http://jeu_pong:5000" //à changer en fonction du port nginx
...
GameData	get_game_data() {
	CURL *curl;
	CURLcode res;
	char pong_data[4096] = {0};

	curl = curl_easy_init();
	if(curl) {
		curl_easy_setopt(curl, CURLOPT_URL, GAME_API_URL "/game_state");
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_callback);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, pong_data);
		res = curl_easy_perform(curl);
		curl_easy_cleanup(curl);
	}

	cJSON *json = cJSON_Parse(pong_data);
	GameData data;
	cJSON *ball_position = cJSON_GetObjectItem(json, "ball_position");
	cJSON *paddle_speed = cJSON_GetObjectItem(json, "paddle_speed");
	cJSON *ball_speed = cJSON_GetObjectItem(json, "ball_speed");
	cJSON *paddle_position = cJSON_GetObjectItem(json, "paddle_position");
	...
	data.field_x = cJSON_GetObjectItem(ball_speed, "x")->valueint;
	data.ball_speed_y = cJSON_GetObjectItem(ball_speed, "y")->valueint;
	data.paddle_x = cJSON_GetObjectItem(paddle_position, "x")->valueint;
	data.paddle_y = cJSON_GetObjectItem(paddle_position, "y")->valueint;
	data.field_x = cJSON_GetObjectItem(field, "x")->valueint;
	data.field_y = cJSON_GetObjectItem(field, "y")->valueint;
...
int	 predict_ball_paddle_intersection(GameData data)
{
	int intersectionX = data.field_x / 2 - data.paddle_x / 2;
	int intersectionY; //value we are looking for

	int distanceToIntersection = intersectionX - data.ball_x;
	int timeToIntersection = distanceToIntersection / data.ball_speed_x;
	
	int intersectionWithoutRebounds = data.ball_y + (data.ball_speed_y * timeToIntersection);
...
int	ai_action(GameData data) {
	...
	int predicted_intersection = predict_ball_paddle_intersection(data);
